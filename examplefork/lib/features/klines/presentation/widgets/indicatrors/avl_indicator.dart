...
class AVLPaintObject extends SinglePaintObjectBox<AVLIndicator> {
  AVLPaintObject({required super.controller, required super.indicator});
  ...
  void paintChart(Canvas canvas, Size size) {
    ...
    for (var i = start; i < end; i++) {
      ...
      if (m.volCcy != null && !m.vol.isZero) {
        points.add(
          Offset(indexToDx(i, check: false)! - candleWidthHalf, valueToDy(m.volCcy!.div(m.vol))),
        );
      }
    }
  ...
  @override
  Size? paintTips(Canvas canvas, {CandleModel? model, Offset? offset, Rect? tipsRect}) {
