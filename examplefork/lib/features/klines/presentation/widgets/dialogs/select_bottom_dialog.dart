...
class SelectBottomDialog<T> extends ConsumerWidget {
  const SelectBottomDialog({super.key, required this.title, required this.list, this.current});
  ...
  static Future<T?> show<T>({required String title, required List<T> list, T? current}) async {
    return DialogManager().showBottomDialog(
      dialogTag: dialogTag(title),
      builder: (context) => SelectBottomDialog(title: title, list: list, current: current),
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Container(
            padding: EdgeInsetsDirectional.symmetric(horizontal: 16.r, vertical: 12.r),
          ...
          ...list.map(
            (e) => ListTile(
              ...
              trailing: current == e ? Icon(Icons.check_outlined, size: 24.r) : null,
            ),
          ),
        ],
      ),
