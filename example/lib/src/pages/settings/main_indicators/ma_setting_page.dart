...
class _MASettingPageState extends BaseIndicatorSettingPageState<MASettingPage> {
  ...
  Widget _buildMALineRow(int index, MALineConfig line, FKTheme theme) {
    return Container(
  ...
  void _toggleLine(int index) {
    final lines = List<MALineConfig>.from(_currentParam.lines);
  ...
  void _updateLineQuietly(int index, MALineConfig newLine) {
    final lines = List<MALineConfig>.from(_currentParam.lines);
    lines[index] = newLine;
    _currentParam = _currentParam.copyWith(lines: lines);
  }

  void _updateLine(int index, MALineConfig newLine) {
    final lines = List<MALineConfig>.from(_currentParam.lines);
    lines[index] = newLine;
    setState(() {
      _currentParam = _currentParam.copyWith(lines: lines);
  ...
  void _addNewLine() {
    ...
    final colors = _currentParam.defaultColors;
    final nextColor = colors[lines.length % colors.length];

    lines.add(MALineConfig(
      id: 'ma_$newPeriod',
      enabled: true,
      period: newPeriod,
      color: nextColor,
