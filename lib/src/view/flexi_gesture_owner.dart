...
/// 排他模式（drawDrawing、cross）→ 命中具体对象（businessOverlay、drawEditing、paintObject）
/// → 区域性辅助操作（zoomSlider、zoomingMove，判据只有「落点在不在某个 Rect 里」）→ 全局兜底。
enum FlexiGestureOwner {
  ...
  static FlexiGestureOwner? resolveLanded(FlexiKlineController controller, Offset position) {
    ...
    if (controller.hitTestBusinessOverlayDragStart(position)) return businessOverlay;
  ...
  bool get isChartFallback => switch (this) {
        ...
        drawDrawing || businessOverlay || drawEditing || cross || paintObject || zoomSlider || zoomingMove => false,
      };
  ...
  bool get suppressesLongPress => switch (this) {
        ...
        businessOverlay || drawEditing || paintObject || zoomSlider || zoomingMove || chartScale || chartPan => false,
      };
}
