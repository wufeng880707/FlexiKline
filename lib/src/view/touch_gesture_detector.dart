...
class _TouchGestureDetectorState extends GestureDetectorState<TouchGestureDetector> {
  ...
  ({GestureData data, Offset origin})? _startDrive(
    ...
    switch (owner) {
      ...
      case FlexiGestureOwner.businessOverlay:
        final data = GestureData.pan(origin);
        if (!controller.onBusinessOverlayDragStart(data)) return null;
        logd('onScaleStart businessOverlay drag down:$down');
        _stopPositionAnimation();
  ...
  void onLongPressStart(LongPressStartDetails details) {
    ...
    if (controller.businessOverlayState.isEditing && controller.onBusinessOverlayDragStart(businessLongData)) {
      logd('onLongPressStart businessOverlay drag > details:$details');
