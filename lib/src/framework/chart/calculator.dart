...
abstract interface class IndicatorCalculationScope<T extends ComputedIndicator> implements ILogger {
  T get indicator;
...
abstract class ComputedIndicatorCalculator<T extends ComputedIndicator> extends IndicatorCalculator<T>
    with FlexiLog
    implements IndicatorCalculationScope<T> {
  ComputedIndicatorCalculator(super.indicator, super.dataIndex);

  late KlineData _klineData;
  ...
  void compute(KlineData data, Range range, {bool reset = false}) {
    _klineData = data;
    computeIndicatorData(range, reset: reset);
  }
